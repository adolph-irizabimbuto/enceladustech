...
function generateWebfilter(URLs){
    ...
    TEXTAREA.innerText = ""
    TEXTAREA.append("config webfilter urlfilter\n");
    TEXTAREA.append(`edit ${wf_number}\n`)
    TEXTAREA.append("config entries\n")
    
    for (let i = 1; i < URLs.length; i++){
        let SANITISED_URL = URLs[i].trim()
        if(SANITISED_URL.length <= 0){
            continue 
         }
        else {
            SANITISED_URL = SANITISED_URL.replace(/\s+$/, '');
...
//EVENT LISTENER
    MY_FORM.addEventListener("submit", function (e) {
       ...
       reader.onload = function (e) {
         ...
         else if(WEBFILTER.checked){
        generateWebfilter(IP_ADDRESS)
         }
        }
